...
class VoxelRobot:
    ...
    def save_model(self, filename) -> None:
        """Save the current voxel grid to a .parquet file of a given name"""
        data = []
    ...
    def load_model(self, filename) -> None:
        """Reset the current voxel grid to all zeroes and replace with a .parquet file"""
    ...
    def generate_model(self) -> tuple:
        ...
        points_grid = np.zeros((self.max_x+1, self.max_y+1, self.max_z+1), dtype=np.int8)
        points_count = 0
        points_map = {}
        # (0, 1, 5) : 24
    ...
    def _is_point_vertex(self, x, y, z) -> bool:
        ...
        return (self._is_point_possible(x, y, z) and self.grid[x, y, z] == 1) or \
            (self._is_point_possible(x-1, y, z) and self.grid[x-1, y, z] == 1) or \
            (self._is_point_possible(x-1, y-1, z) and self.grid[x-1, y-1, z] == 1) or \
            (self._is_point_possible(x, y-1, z) and self.grid[x, y-1, z] == 1) or \
            (self._is_point_possible(x, y, z-1) and self.grid[x, y, z-1] == 1) or \
            (self._is_point_possible(x-1, y, z-1) and self.grid[x-1, y, z-1] == 1) or \
            (self._is_point_possible(x-1, y-1, z-1) and self.grid[x-1, y-1, z-1] == 1) or \
            (self._is_point_possible(x, y-1, z-1) and self.grid[x, y-1, z-1] == 1)
...
class VoxelRobot2:
    def __init__(self, grid) -> None:
        if not isinstance(grid, np.ndarray) or grid.shape != (10, 10, 10):
            raise ValueError("Grid must be a 10x10x10 numpy array.")
        ...
        if not self.is_contiguous():
            raise ValueError("The Voxel structure is not contiguous.")

    def visualise(self) -> None:
        fig = plt.figure()
        ax = fig.add_subplot(111, projection="3d")

        # Extract coordinates of all 1's in the grid
        x, y, z = np.where(self.grid == 1)

        # Define voxel size
        voxel_size = 0.9  # slightly smaller than 1 to avoid overlap
        ...
        # Set limits to maintain 10x10x10 cube view
        ax.set_xlim(0, 10)
        ax.set_ylim(0, 10)
        ax.set_zlim(0, 10)
        ...
        # Set grid lines to be per 1 unit
    ...
    def generate_model(self) -> tuple:
        # use generate voxel for each grid point that is 1
        # generate the points and elements
        ...
        for x in range(10):
            for y in range(10):
                for z in range(10):
                    if self.grid[x, y, z] == 1:
                        p, e = self._generate_voxel(x, y, z)
                        points_string += p
                        element_string += e
    ...
    def _generate_voxel(self, x: int, y: int, z: int) -> tuple:
        points = [
            (x, y, z), # 0
            (x+1, y, z), # 1
            (x+1, y+1, z), # 2
        ...
        for p in points:
            points_string += str(p[0]) + " " + str(p[1]) + " " + str(p[2]) + "   "
        
        for e in elements:
            element_string += " ".join(map(str, e)) + "    "

        return points_string, element_string
