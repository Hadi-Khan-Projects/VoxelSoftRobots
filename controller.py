...
class DistributedNeuralController:
    ...
    def __init__(self, n_voxels, voxel_coords, n_sensors_per_voxel, n_comm_channels, weights, biases, driving_voxel_coord=None, time_signal_frequency=1.0):
        """
        ...
        self.input_size = self.n_sensors + self.N_COMM_DIRECTIONS * self.n_comm + 1 + self.N_TIME_INPUTS
        ...
        if weights.shape != expected_weights_shape:
            raise ValueError(f"Weights shape mismatch. Expected {expected_weights_shape}, got {weights.shape}")
        if biases.shape != expected_biases_shape:
            raise ValueError(f"Biases shape mismatch. Expected {expected_biases_shape}, got {biases.shape}")
        ...
        self.previous_comm_outputs = np.zeros((self.n_voxels, self.N_COMM_DIRECTIONS, self.n_comm))
        # To store outputs calculated in the current step before they become 'previous'
        self.current_comm_outputs_buffer = np.zeros((self.n_voxels, self.N_COMM_DIRECTIONS, self.n_comm))
        # ---------------------
    ...
    def get_neighbor_comm_input(self, voxel_index, direction_offset):
        ...
        if neighbor_coord in self.voxel_coord_to_index:
            ...
            elif direction_offset == (0, 1, 0): # Our Up input (Y+) needs neighbor's Down output (Y-)
                neighbor_output_side_index = self.COMM_IDX_D
            elif direction_offset == (0, -1, 0):# Our Down input (Y-) needs neighbor's Up output (Y+)
                neighbor_output_side_index = self.COMM_IDX_U
            else:
                 # Invalid offset for communication
                 # print(f"Warning: Communication direction offset {direction_offset} not handled.")
            ...
            if 0 <= neighbor_output_side_index < self.N_COMM_DIRECTIONS:
                return self.previous_comm_outputs[neighbor_index, neighbor_output_side_index, :]
                 ...
                 print(f"Warning: Invalid neighbor output side index {neighbor_output_side_index} calculated for offset {direction_offset}.")
                 return np.zeros(self.n_comm)
        else:
    ...
    def step(self, sensor_data_all_voxels, time):
        ...
        for i in range(self.n_voxels):
            ...
            comm_inputs_flat = np.concatenate([comm_input_N, comm_input_E, comm_input_S, comm_input_W, comm_input_U, comm_input_D])
            ...
            if mlp_input.shape[0] != self.input_size:
                 raise RuntimeError(f"MLP input size mismatch for voxel {i}. Expected {self.input_size}, got {mlp_input.shape[0]}. Check sensor/comm/driving/time components.")
            ...
            start_idx = 1
            comm_out_N = mlp_output[start_idx : start_idx + self.n_comm]; start_idx += self.n_comm
            comm_out_E = mlp_output[start_idx : start_idx + self.n_comm]; start_idx += self.n_comm
            comm_out_S = mlp_output[start_idx : start_idx + self.n_comm]; start_idx += self.n_comm
            comm_out_W = mlp_output[start_idx : start_idx + self.n_comm]; start_idx += self.n_comm
            comm_out_U = mlp_output[start_idx : start_idx + self.n_comm]; start_idx += self.n_comm
            comm_out_D = mlp_output[start_idx : start_idx + self.n_comm]; #start_idx += self.n_comm # No need to increment last one
    ...
    def load_parameters(self, weights, biases):
        ...
        expected_biases_shape = (self.output_size,)
        if weights.shape != expected_weights_shape:
            raise ValueError(f"Weight shape mismatch. Expected {expected_weights_shape}, got {weights.shape}")
        if biases.shape != expected_biases_shape:
            raise ValueError(f"Biases shape mismatch. Expected {expected_biases_shape}, got {biases.shape}")
